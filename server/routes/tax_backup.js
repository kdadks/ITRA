...
router.put('/returns/:id/deductions', auth, async (req, res) => {
  try {
    ...
    const { deductions } = req.body;
    taxReturn.deductions = { ...taxReturn.deductions, ...deductions };
    
    // Calculate total deductions
    const totalDeductions = 
      (taxReturn.deductions.section80C || 0) +
      (taxReturn.deductions.section80D || 0) +
      (taxReturn.deductions.section80G || 0) +
      (taxReturn.deductions.section80E || 0) +
      (taxReturn.deductions.section80TTA || 0) +
      (taxReturn.deductions.otherDeductions || 0);
    
    taxReturn.deductions.totalDeductions = totalDeductions;
    taxReturn.updatedAt = new Date();
...
router.post('/returns/:id/calculate', auth, async (req, res) => {
  try {
    ...
    const taxComputations = taxReturn.calculateTaxLiability();
...
// Update tax payments (TDS/Advance Tax)
router.put('/returns/:id/payments', auth, async (req, res) => {
  try {
    const taxReturn = await TaxReturn.findOne({
      _id: req.params.id,
      userId: req.user.userId
    ...
    const { tdsDeducted, advanceTaxPaid } = req.body;
    
    taxReturn.taxComputations.tdsDeducted = tdsDeducted || 0;
    taxReturn.taxComputations.advanceTaxPaid = advanceTaxPaid || 0;
    taxReturn.updatedAt = new Date();

    // Recalculate refund/additional payment
    taxReturn.calculateTaxLiability();

    await taxReturn.save();

    res.json({
      message: 'Tax payments updated successfully',
      taxReturn
    });
  } catch (error) {
...
// Update bank details
router.put('/returns/:id/bank-details', auth, async (req, res) => {
  try {
    const taxReturn = await TaxReturn.findOne({
      _id: req.params.id,
      userId: req.user.userId
    ...
    taxReturn.bankDetails = { ...taxReturn.bankDetails, ...bankDetails };
    taxReturn.updatedAt = new Date();
...
router.post('/returns/:id/file', auth, async (req, res) => {
  try {
    ...
    if (taxReturn.status === 'draft') {
      return res.status(400).json({ 
        message: 'Please calculate tax before filing' 
      });
    }
    ...
    taxReturn.acknowledgmentNumber = `ITR${Date.now()}${Math.floor(Math.random() * 1000)}`;
    taxReturn.updatedAt = new Date();
...
// Get tax summary/dashboard data
router.get('/dashboard', auth, async (req, res) => {
  try {
    const currentYear = new Date().getFullYear();
    const assessmentYear = `${currentYear}-${currentYear + 1}`;
    
    // Get current year's return
    const currentReturn = await TaxReturn.findOne({
      userId: req.user.userId,
      assessmentYear
    });

    // Get all returns for statistics
    const allReturns = await TaxReturn.find({ userId: req.user.userId });
    ...
    const dashboardData = {
      ...
      pendingReturns: allReturns.filter(r => r.status === 'draft').length,
      totalRefunds: allReturns.reduce((sum, r) => sum + (r.taxComputations.refundDue || 0), 0),
      totalTaxPaid: allReturns.reduce((sum, r) => sum + (r.taxComputations.taxLiability || 0), 0)
    };
